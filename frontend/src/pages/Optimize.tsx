...
import {
  ...
  RotateCcw, Copy, Code2, Eye, ArrowRight, Building2, Info, X,
} from "lucide-react";
    ...
    <div>
      <div className="flex items-center justify-between mb-1.5">
        ...
        <div className="flex items-center gap-2">
          {delta !== null && delta !== 0 && (
            <span className="text-[10px]" style={{ color: delta > 0 ? "#4ade80" : "#f87171" }}>
              {delta > 0 ? "+" : ""}{delta}
            </span>
          )}
          <span className="text-sm font-manrope font-medium" style={{ color }}>{value}</span>
        </div>
      </div>
...
function ChangeBadge({ category }: { category: string }) {
  ...
  return (
    <span className="shrink-0 text-[9px] font-manrope font-semibold uppercase tracking-widest px-1.5 py-0.5 rounded"
    ...
    <div className="rounded-xl overflow-hidden" style={{ background: "rgba(22,24,28,0.6)", border: "1px solid rgba(68,71,72,0.4)" }}>
      <button onClick={onToggle}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[11px] font-manrope font-semibold text-text-secondary uppercase tracking-[0.15em]">
            What Changed
          </span>
          {count > 0 && (
            <span className="text-[9px] px-1.5 py-0.5 rounded font-manrope"
            ...
            <div className="px-4 pb-4" style={{ borderTop: "1px solid rgba(68,71,72,0.3)" }}>
              ...
              <div className="flex flex-wrap gap-1.5 py-3 mb-1">
                {Object.entries(CHANGE_STYLES).filter(([cat]) => grouped[cat]?.length).map(([cat, s]) => (
                  <span key={cat} className="text-[9px] px-1.5 py-0.5 rounded font-manrope uppercase tracking-widest"
                    style={{ color: s.color, background: s.bg, border: `1px solid ${s.border}` }}>
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto no-scrollbar pr-1">
                {displayItems.map((item, i) => (
                  <motion.div key={i}
                    ...
                    <span className="text-[11px] text-text-secondary font-light leading-relaxed">{item.text}</span>
                  </motion.div>
                ))}
              </div>
            </div>
      ...
      <button onClick={onToggle}
        ...
        <div className="flex items-center gap-2.5">
          ...
          {result.ats_score_after != null && (
            <span className="text-[10px] font-manrope font-medium px-1.5 py-0.5 rounded"
              style={{
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
            ...
            <div className="px-5 pb-5 space-y-4" style={{ borderTop: "1px solid rgba(68,71,72,0.3)" }}>
              ...
              {result.matched_keywords.length > 0 && (
                <div>
                  <p className="text-[10px] text-text-dim uppercase tracking-widest mb-2">
                ...
                <div>
                  ...
                  <ul className="space-y-1.5 mb-3">
                    {result.improvements.map((imp, i) => (
                      <li key={i} className="flex items-start gap-2 text-[11px] text-text-secondary font-light">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2 rounded-lg text-[11px] font-manrope font-medium transition-all"
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ background: "rgba(17,19,23,0.8)", border: "1px solid rgba(68,71,72,0.4)" }}>
      ...
      <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid rgba(68,71,72,0.3)" }}>
        <div className="flex gap-1">
          {([
            ...
            <button key={v} onClick={() => setView(v as any)}
              className="flex items-center gap-1.5 px-2.5 py-1 rounded text-[10px] font-manrope uppercase tracking-wide transition-all"
              style={view === v
        ...
        <button onClick={handleCopy} className="flex items-center gap-1 text-[10px] text-text-muted hover:text-text-secondary transition-colors">
          <Copy className="w-3 h-3" />Copy
        </button>
      </div>

      {/* Content */}
      <pre className="text-[11px] text-text-secondary leading-relaxed whitespace-pre-wrap px-5 py-4 max-h-[460px] overflow-y-auto font-mono no-scrollbar">
        {displayContent}
      </pre>

      {/* Footer actions */}
      <div className="flex gap-2 px-4 pb-4 pt-2 flex-wrap" style={{ borderTop: "1px solid rgba(68,71,72,0.25)" }}>
        <button onClick={handleDownloadTxt} className="btn-ghost flex-1 flex items-center justify-center gap-1.5 text-[11px] py-2">
          <Download className="w-3 h-3" />.txt
        </button>
        {result.latex_text && (
          <button onClick={handleDownloadLatex}
            className="btn-ghost flex-1 flex items-center justify-center gap-1.5 text-[11px] py-2"
        ...
        <button onClick={onReoptimize} disabled={reoptLoading}
          className="btn-ghost flex items-center gap-1.5 text-[11px] py-2 px-4"
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex items-end gap-4 flex-wrap">
          <div>
            ...
            <p className="text-text-secondary mt-1.5 font-light text-sm">
              World-class rewrite engine — strategic psychology, keyword science, LaTeX export.
            </p>
          </div>
          {passNum > 1 && (
            <span className="mb-1 text-[10px] px-2.5 py-1 rounded-full font-manrope uppercase tracking-widest"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-6 items-start">
        ...
        <div className="space-y-4 lg:sticky lg:top-24">
          ...
          <div className="glass-card-static p-5">
            <p className="text-[11px] font-manrope font-semibold text-text-secondary uppercase tracking-[0.15em] mb-4 flex items-center gap-2">
          ...
          <div className="glass-card-static p-5 space-y-4">
            <p className="text-[11px] font-manrope font-semibold text-text-secondary uppercase tracking-[0.15em]">
              Target Job
            </p>

            {/* URL fetch */}
            <div>
              <p className="text-[10px] text-text-muted uppercase tracking-widest mb-1.5 font-manrope">Job URL</p>
              <div className="flex gap-2">
                <input type="url" className="input-base text-xs flex-1"
              ...
              {fetchedMeta && (
                <div className="flex items-center gap-1.5 mt-1.5">
                  ...
                  <span className="text-[10px] text-text-muted">{[fetchedMeta.title, fetchedMeta.company].filter(Boolean).join(" · ")}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-text-dim">or paste</span>
              <div className="flex-1 h-px" style={{ background: "rgba(68,71,72,0.4)" }} />
            </div>
            ...
            <textarea className="input-base min-h-[160px] resize-none font-mono text-xs leading-relaxed"
              placeholder="Paste the full job description…"
              value={jobDesc} onChange={e => setJobDesc(e.target.value)} />
          </div>

          <button className="btn-primary w-full py-3 text-sm" onClick={handleOptimize} disabled={loading || reoptLoading}>
          ...
          {!result && !loading && (
            <div className="rounded-xl p-12 text-center flex flex-col items-center gap-5"
              ...
              <div>
                ...
                <p className="text-text-muted text-xs mt-1">Strategic rewrite · ATS scoring · LaTeX export</p>
              </div>
            </div>
          )}
          ...
          {loading && (
            <div className="rounded-xl p-12 text-center flex flex-col items-center gap-5"
              ...
              <div>
                ...
                <p className="text-text-muted text-xs mt-1">Applying strategic framing, keyword science, and ATS alignment</p>
              </div>
            </div>
          )}
          ...
          {result && (
            <motion.div key={passNum} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
              ...
              {result.linkedin_unavailable && (
                <div className="flex items-start gap-2.5 px-4 py-3 rounded-xl text-[11px] font-inter"
